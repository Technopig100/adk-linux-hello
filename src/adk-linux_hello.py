...
class EmbedBrowser(Embed):
    ...
    def load(self, window: Gtk.Window) -> bool:
        try:
            ...
            try:
                conf = HelloConfig(application="adk-linux-hello")
                grid = Gtk.Grid()
...
class Hello(Gtk.Window):
    ...
    def __init__(self):
        Gtk.Window.__init__(self, title="ADK-Linux Hello", border_width=6)
        self.app = "adk-linux-hello"
...
def get_lsb_infos():
    ...
    try:
        ...
        print(error)
        return 'not ADK-Linux', '0.0'
    # return lsb["CODENAME"], lsb["RELEASE"]
    # work without needing to change /etc/lsb-release on ADK-Linux
